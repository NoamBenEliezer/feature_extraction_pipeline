function[ROI_list_LR, Slice_labels_LR , qMRI_map_3D_LR, Seg_ROI_3D_LR]... %ROI_list_LR, Slice_labels_LR , qMRI_map_3D_LR, Seg_ROI_3D_LR
...
Top_9_list={'Cerebral-White-Matter','Caudate', 'Putamen','Pallidum', 'Thalamus-Proper*', 'VentralDC', 'Accumbens-area', ...
    'Amygdala', 'Hippocampus','ctx%sinsula', 'ctx%s'}; % Out - CC, VentralDC, Brain-Stem, Cerebelum (WM and cortex), cortex (all) = 'ctx%s'
...
ROIs_loc=[];
excluded_ROIs=[];
for i= 1:length(Top_9_list)
    for j=1:length (Top_LR_list)
        
        if strcmp(Top_LR_list{j}, '_all')
            curr_name=[Top_9_list{i} Top_LR_list{j}];
            if strfind (Top_9_list{i}, 'ctx')
                curr_name=[sprintf(Top_9_list{i},"_") Top_LR_list{j}];
            end
        else
            curr_name=[Top_LR_list{j} Top_9_list{i}];
            if strfind (Top_9_list{i}, 'ctx')
                if strcmp(Top_LR_list{j}, 'Left-')
                    curr_name=[sprintf(Top_9_list{i},"-lh-")];
                    if Top_9_list{i}=="ctx%s"
...
%                             std(ROI_list_all(ROIS_loc_ctx_list ,3)) 100*std(ROI_list_all(ROIS_loc_ctx_list ,3))/mean(ROI_list_all(ROIS_loc_ctx_list ,3))];
                ...
                elseif strcmp(Top_LR_list{j}, 'Right-')
                    curr_name=[sprintf(Top_9_list{i},"-rh-")];
                    if Top_9_list{i}=="ctx%s"
...
% for idx_ROI=1:length(Top_9_list)
    if ~Top_9_list{end}=='ctx%s'
